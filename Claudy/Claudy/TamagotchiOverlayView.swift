...
// MARK: - TamagotchiHoverStatsView

/// A compact floating pill that shows Claud-y's three Tamagotchi stats on hover.
/// Appears above the character, fades in smoothly, never conflicts with the speech bubble
/// because it lives as an overlay *inside* the character ZStack rather than the VStack.
struct TamagotchiHoverStatsView: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            statPip(icon: "fork.knife",   value: manager.fullness,   color: orange)
            divider
            statPip(icon: "face.smiling", value: manager.happiness,  color: .yellow)
            divider
            statPip(icon: "bolt.fill",    value: manager.energy,     color: Color(red: 0.3, green: 0.85, blue: 0.95))
        }
    ...
    private func statPip(icon: String, value: Float, color: Color) -> some View {
        HStack(spacing: 4) {
...
/// Thin wrapper: reads overlay pref + hover state, shows/hides the hover stats.
struct TamagotchiHoverStatsIfEnabled: View {
    let manager: TamagotchiManager
    let isHovered: Bool
    @AppStorage(DefaultsKeys.tamagotchiOverlayEnabled) private var overlayEnabled = false

    var body: some View {
        if overlayEnabled && isHovered {
            TamagotchiHoverStatsView(manager: manager)
