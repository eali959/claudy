...
- AI: Codex (Anthropic) · ChatGPT (OpenAI) · Gemini (Google) — all via REST, no SDK dependency
...
|----------|------|-----------|------------|---------------|-------------|
| `.Codex` | `x-api-key` header | SSE `content_block_delta` | `Codex-3-5-haiku-20241022` | `Codex-haiku-4-5-20251001` | `Codex-sonnet-4-6` |
...
- API keys stored in macOS Keychain via `KeychainService` — per-provider accounts (`Codex-api-key`, `openai-api-key`, `gemini-api-key`)
...
| Key | Type | Notes |
|-----|------|-------|
| `SelectedProvider` | String | `"Codex"` / `"openai"` / `"gemini"` |
